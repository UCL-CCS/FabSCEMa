...
def init_run_analyse_campaign(work_dir=None, sampler_inputs_dir=None , inpt=None):
    ...
    for string_l in output_column:
        arg1, arg2 = ess_result_output(CEND, CRED, campaign, campaign_work_dir, output_column, sampler,
                                       sur_campaign, srtn=string_l)
        # idx_t.append(arg2)
    ...
    # meanx = np.mean(np.array(mean_grad_t))
    mean_all = mean_all / len(output_column)
    idxx = np.fliplr(np.argsort(mean_all.T))
    params_ordered = np.array(list(sampler.vary.get_keys()))[idxx[0]]
    ...
    ax.bar(range(mean_all.size), height=mean_all[idxx].flatten())
    ax.set_xticks(range(mean_all.size))
...
def sensitivity_measures(surrogate, feats, norm=True):
    # feats = (feats - np.mean(feats)) / np.std(feats)
    feats = (feats - surrogate.feat_mean) / surrogate.feat_std
    ...
    for i in range(N):
        ...
        # print("feats[i].reshape([1, -1]", feats[i].reshape([1, -1]))
        df_dx = surrogate.neural_net.d_norm_y_dX(feats[i].reshape([1, -1]),
                                                 norm=norm)
...
def ess_result_output(CEND, CRED, campaign, campaign_work_dir, output_column, sampler, sur_campaign, srtn=None):
    ...
    n_iter = 20000
    surrogate.train(params_su, samples_su, n_iter, n_layers=4, n_neurons=256,
                    batch_size=512, test_frac=0.2)
    ...
    # analysis.get_classification_error()
    ...
    new_samples = feat_train.shape[0]

    prediction = np.zeros([new_samples, samples_su.shape[1]])
    for i in range(new_samples):
        prediction[i, :] = surrogate.predict(params_su[i])
        print("prediction[i, :]", prediction[i, :])
    train_data = samples_su[0:feat_train.shape[0]]
    rel_err_train = np.linalg.norm(train_data - prediction) / np.linalg.norm(train_data)
    prediction = np.zeros([feat_test.shape[0], samples_su.shape[1]])
    for idx, i in enumerate(range(feat_train.shape[0], samples_su.shape[0])):
        prediction[idx] = surrogate.predict(params_su[i])

    test_data = samples_su[feat_train.shape[0]:]
    ...
    idx, mean_grad = sensitivity_measures(surrogate, params_nw)
