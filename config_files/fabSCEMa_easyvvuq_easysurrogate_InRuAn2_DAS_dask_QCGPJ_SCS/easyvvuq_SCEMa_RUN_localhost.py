...
number_of_sampling_steps_nano = 1
len_qpid_macro = 64
if __name__ == '__main__':
    ...
    try:
        ...
        columns_to_be_removed1 = ['timestep', 'time', 'qpid', 'cell', 'qpoint', 'material', 'strain_00', 'strain_01',
                                  'strain_02', 'strain_11', 'strain_12', 'strain_22', 'updstrain_00', 'updstrain_01',
                                  'updstrain_02', 'updstrain_11', 'updstrain_12', 'updstrain_22']
        ...
        print('macro data shape:', data_macro2.shape)
        X_macro = []
        for i in range(data_macro2.shape[1]):
            tmp = []
            for j in range(int((data_macro2.shape[0] + 1) / len_qpid_macro)):
                tmp.append(
                    (np.sum(data_macro2[j * len_qpid_macro:j * len_qpid_macro + len_qpid_macro, i])) / len_qpid_macro)
    ...
    try:
        ...
        columnsm = ['index', 'stress_00_nano', 'stress_01_nano', 'stress_02_nano', 'stress_11_nano', 'stress_12_nano',
                    'stress_22_nano']
        m_l = int((data_macro2.shape[0] + 1) / len_qpid_macro)
        for i in range(len_qpid_macro):
            ...
            if np.array(data_nano).shape[0] < m_l:
                # name = 'output_nano{}.csv'.format(i)
                # print(name)
                data_n = pd.read_csv('output_nano{}.csv'.format(i))
                data_n = np.array(data_n)
                # print('data_n shape:', data_n.shape)
                input_array = np.array(np.zeros_like(np.arange(data_n.shape[1])))
                new_array = np.vstack([input_array, data_n])
                # print('new_array shape:', new_array.shape)
                np.savetxt('output_nano{}.csv'.format(i), new_array, delimiter=","
                           ,
                           header="stress_00_nano,stress_01_nano,stress_02_nano,stress_11_nano,stress_12_nano,stress_22_nano",
                           comments='')
    ...
    try:
        ...
        for i in range(len_qpid_macro):
            ...
            print('nano data shape:', data_nano1.shape)
            X1_nano = []
    ...
    try:
        ...
        columnsf = ['index', 'stress_00_macro', 'stress_01_macro', 'stress_02_macro', 'stress_11_macro',
                    ...
                    'stress_11_nano', 'stress_12_nano',
                    'stress_22_nano']
        # columnsr = ['index', 'stress_00_macro', 'stress_01_macro', 'stress_02_macro', 'stress_11_macro', 'stress_12_macro', 'stress_00_nano', 'stress_01_nano', 'stress_02_nano', 'stress_11_nano',
        #             'stress_12_nano']
        columnsr = ['index']
        ...
        data_macrox.to_csv('output.csv',
                           index=None, header=False)
        # cols = ['stress_22_macro', 'stress_22_nano']
