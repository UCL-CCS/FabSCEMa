...
def sensitivity_measures(surrogate, feats, dm, feat_train):
        N = feats.shape[0]
        print("feats.shape", N)
        x = np.arange(dm)
        ...
        for i in range(N):
            ...
            for i in range(len(tmp[:, 0])):
                ...
                # print("A", A.shape)
                # # X = tmp[:, 0][i].reshape(1, -1) - surrogate.X_train.reshape(1, -1)
                # print("feat_train", feat_train[:, i].shape)
                # print("tmp[:, 0][i].reshape(1, -1)", np.array(tmp[:, 0][i].reshape(1, -1)).shape)
                # # X = tmp[:, 0][i].reshape(1, -1) - feat_train[:, i](1, -1)
                X = tmp[:, 0][i] - feat_train[:, i]
                # print("X", np.array(X).shape)
                # X = tmp[:, 0][i].reshape(1, -1) - surrogate.X_train.reshape(1, -1)
                f = np.exp(-(X) ** 2 / (2 * length_scale ** 2))
                df = (f * (-X / length_scale ** 2))
                df_dx.append(np.sum((constant_value * df @ Alpha / N)))

            mean = list(map(add, mean, df_dx))

        # mean = normalize(mean[:,np.newaxis], axis=0).ravel()
        print("mean", mean)
...
def ess_result_output(campaign, campaign_work_dir, output_column, sampler, sur_campaign, srtn=None):
    ...
    surrogate = es.methods.GP_Surrogate(n_in=D1, n_out=S1)
    print('Time to initialise the surrogate: {:.3} s'.format(t.time() - time_init_start))
    time_train_start = t.time()
    surrogate.train(feats=params_su, target=samples_su, test_frac=0.2, basekernel='RBF')
    ...
    idx, mean_grad = sensitivity_measures(surrogate, params_nw, D1, feat_train)
