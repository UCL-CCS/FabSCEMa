...
def init_run_analyse_campaign(work_dir=None, sampler_inputs_dir=None , inpt=None):
    ...
    # n_samples = campaign._active_sampler.n_samples()
    ...
    params_su, samples_su = sur_campaign.load_easyvvuq_data(campaign, qoi_cols=["resulting_force"])
    samples_su = samples_su[output_column[0]]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(campaign_work_dir, '{}_sensitivity_surrogate.png'.format("resulting_force")))
    print("saving {}_sensitivity_surrogate -->".format("resulting_force"),
          os.path.join(campaign_work_dir, '{}_sensitivity_surrogate.png'.format("resulting_force")))
    # plt.show()
